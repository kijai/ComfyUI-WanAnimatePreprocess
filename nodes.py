...
from .visualization import joints_dict
...
class BodyRegions:
    """Derives skeleton region groupings from :func:`visualization.joints_dict`.

    The mapping is built dynamically from the reference definitions so that the
    node stays aligned with the official keypoint naming.  We support the
    ``coco`` (17 point), ``coco_25`` (25 point) and ``wholebody`` (133 point)
    layouts.  If an unknown keypoint count is encountered we fall back to the
    closest available definition.
    """

    def __init__(self) -> None:
        joints = joints_dict()
        self._by_length: Dict[int, _RegionDefinition] = {}

        for layout_name in ("wholebody", "coco_25", "coco"):
            layout = joints.get(layout_name) or {}
            keypoints = layout.get("keypoints")
            if not keypoints:
                continue
            region = self._build_regions(keypoints)
            self._by_length[len(keypoints)] = region

        if not self._by_length:
            raise RuntimeError("No skeleton layouts available to build region definitions.")

        # Store the layout that is closest to each count for quick lookup.
        self._sorted_lengths = sorted(self._by_length.keys())

    def resolve(self, keypoint_count: int) -> _RegionDefinition:
        if keypoint_count in self._by_length:
            return self._by_length[keypoint_count]

        # Find the registered layout with the smallest difference in size.
        closest_length = min(self._sorted_lengths, key=lambda value: abs(value - keypoint_count))
        return self._by_length[closest_length]

    def _build_regions(self, keypoints: Mapping[int, str]) -> _RegionDefinition:
        head: List[int] = []
        upper_body: List[int] = []
        hips: List[int] = []
        legs: List[int] = []
        ...
        head_keywords = ("nose", "eye", "ear", "head", "face", "mouth", "lip", "chin", "brow")
        hip_keywords = ("hip", "pelvis", "waist")
        arm_keywords = ("shoulder", "elbow", "wrist", "arm")
        hand_keywords = ("hand", "finger", "thumb")
        torso_keywords = ("neck", "torso", "chest", "spine", "trunk", "body")
        leg_keywords = ("leg", "thigh", "knee", "ankle", "toe", "heel", "calf", "shank")
        foot_keywords = ("ankle", "toe", "heel", "foot", "feet")

        for index, name in keypoints.items():
            idx = int(index)
            label = str(name).lower()

            is_head = any(keyword in label for keyword in head_keywords)
            is_hip = any(keyword in label for keyword in hip_keywords)
            is_arm = any(keyword in label for keyword in arm_keywords)
            is_hand = any(keyword in label for keyword in hand_keywords)
            is_torso = any(keyword in label for keyword in torso_keywords)
            is_leg = any(keyword in label for keyword in leg_keywords)
            is_foot = any(keyword in label for keyword in foot_keywords)

            if is_head or label.startswith("face"):
                head.append(idx)
            ...
            if is_hip:
                ...
                if "left" in label:
                    ...
                    right_hip.append(idx)

            if is_leg or is_foot:
                legs.append(idx)
                if "left" in label:
                    left_leg.append(idx)
                    if is_foot:
                        left_foot.append(idx)
                elif "right" in label:
                    right_leg.append(idx)
                    if is_foot:
                        right_foot.append(idx)
                else:
                    if is_foot:
                        feet.append(idx)
                    continue
            ...
            if is_foot:
                feet.append(idx)
                if "left" in label and idx not in left_foot:
                    left_foot.append(idx)
                if "right" in label and idx not in right_foot:
                    right_foot.append(idx)

            if is_leg and "left" not in label and "right" not in label:
                # Add symmetrical legs to both sides to keep scaling stable.
                left_leg.append(idx)
                right_leg.append(idx)
                if is_foot:
                    left_foot.append(idx)
            ...
            if (is_head or is_hip or is_arm or is_hand or is_torso) and not is_leg:
                upper_body.append(idx)

            if is_hand and "left" in label and idx not in upper_body:
                upper_body.append(idx)
            if is_hand and "right" in label and idx not in upper_body:
                upper_body.append(idx)

            if label.startswith("face") and idx not in upper_body:
                upper_body.append(idx)

        if not hips:
            hips = [idx for idx in upper_body if "hip" in str(keypoints[idx]).lower()]
        ...
        if not left_leg:
            left_leg = [idx for idx in legs if idx % 2 == 1] or legs.copy()
        if not right_leg:
            right_leg = [idx for idx in legs if idx % 2 == 0] or legs.copy()

        if not left_foot:
            left_foot = [idx for idx in feet if idx % 2 == 1] or feet.copy()
        if not right_foot:
            right_foot = [idx for idx in feet if idx % 2 == 0] or feet.copy()

        return _RegionDefinition(
            head=tuple(sorted(set(head))),
            upper_body=tuple(sorted(set(upper_body or head))),
            hips=tuple(sorted(set(hips or upper_body))),
            legs=tuple(sorted(set(legs))),
            feet=tuple(sorted(set(feet))),
            left_leg=tuple(sorted(set(left_leg))),
            right_leg=tuple(sorted(set(right_leg))),
            left_foot=tuple(sorted(set(left_foot))),
            right_foot=tuple(sorted(set(right_foot))),
            left_hip=tuple(sorted(set(left_hip))),
            right_hip=tuple(sorted(set(right_hip))),
        )
...
class PoseDataEditorAloneAutomaticChaty:
    ...
    The component consumes Johnson style pose dictionaries (see
    ``json_format.json``) and outputs a modified sequence in the same format.
    Its behaviour follows the specification provided in the user request:

    * During the configured ``duration`` (in seconds) the node adapts the
      offset/scale so that the upper body respects the configured paddings.
    * Once the duration expires the most recent transform is locked and reused
      for all subsequent frames.
    * ``LockFeet`` keeps ankle/toe keypoints in place while ``ScaleLegs``
      stretches the remaining leg joints to maintain a continuous connection to
      the moving hips.
    """
    ...
    def process_frame(
        ...
        *,
        ...
        for track_id, points in mutable_frame.items():
            ...
            def apply_body_transform(value: float) -> float:
                return self._apply_transform(value, pivot)
    ...
    def _average_visible_y(self, points: List[Optional[Keypoint]], indices: Tuple[int, ...]) -> Optional[float]:
        values: List[float] = []
...
class PoseDataEditorAloneAutomaticChatyNode:
            ...
            "required": {
                ...
                "head_padding": (
                    "FLOAT",
                    {
                        ...
                        "max": 2048.0,
                        "step": 0.01,
                        "tooltip": "Distance to keep between the highest keypoint and the canvas top.",
                    },
                ),
                "foot_padding": (
                    "FLOAT",
                    {
                        ...
                        "max": 2048.0,
                        "step": 0.01,
                        "tooltip": "Distance to keep between the feet and the canvas bottom while scaling legs.",
                    },
                ),
                "normalize": (
                    "BOOLEAN",
                ...
                "head_padding_active_seconds": (
                    "FLOAT",
                    {
                        ...
                        "max": 60.0,
                        "step": 0.01,
                        "tooltip": "How long the head padding remains enforced.",
                    },
                ),
                "foot_padding_active_seconds": (
                    "FLOAT",
                    {
                        ...
                        "max": 60.0,
                        "step": 0.01,
                        "tooltip": "How long the foot padding remains enforced.",
                    },
                ),
                "lock_feet": (
                    "BOOLEAN",
                    {
                        "default": False,
                        "tooltip": "Keep the lowest foot keypoints fixed once the upper body offsets.",
                    },
                ),
                "scale_legs": (
                    "BOOLEAN",
                    {
                        "default": False,
                        "tooltip": "Scale the knees and hips to follow the moving torso when feet are locked.",
                    },
                ),
                "fps": (
                    "INT",
                    {
                        ...
                        "max": 240,
                        "step": 1,
                        "tooltip": "Frame rate used to translate seconds to frames.",
                    },
                ),
            }
        }
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess"
    DESCRIPTION = "Offsets the upper body while optionally locking the feet and scaling the legs."
