...
class NLFProportionalRetargeterV22:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "scale_stance_and_head": ("BOOLEAN", {"default": False, "tooltip": "Wendet den Höhen-Korrekturfaktor auch auf Schulter-/Hüftbreite und den Kopf an."}),
                "temporal_smooth_factor": ("FLOAT", {"default": 0.33, "min": 0.0, "max": 0.99, "step": 0.01, "tooltip": "Glättet die Gelenke gegen Zittern."}),
                "ground_smooth_factor": ("FLOAT", {"default": 0.70, "min": 0.0, "max": 0.99, "step": 0.01, "tooltip": "Glättet NUR die Auf/Ab-Bewegung des Ground Anchors."}),
                ...
                "ground_anchor_mode": (["v18_legacy", "conservative", "advanced_trend"], {"default": "conservative", "tooltip": "V19 Ground-Modus: v18_legacy = altes Verhalten, conservative = Clamp + Body Guard, advanced_trend = Lookahead/Trend-Analyse."}),
                "ground_shift_percentile": ("FLOAT", {"default": 80.0, "min": 50.0, "max": 100.0, "step": 1.0, "tooltip": "Robuster Fußanker statt hartem max()."}),
                "ground_shift_max_step": ("FLOAT", {"default": 14.0, "min": 0.0, "max": 100.0, "step": 1.0, "tooltip": "Maximal erlaubte Änderung des Ground-Shifts pro Frame."}),
                "body_anchor_guard": ("BOOLEAN", {"default": True, "tooltip": "Begrenzt den Ground-Shift, wenn Schulter/Hüfte dadurch zu stark hoch/runter gezogen werden."}),
                "body_anchor_max_delta": ("FLOAT", {"default": 18.0, "min": 0.0, "max": 120.0, "step": 1.0, "tooltip": "Maximale erlaubte Body-Y-Abweichung durch Ground-Shift gegenüber Original/Trend."}),
                "body_anchor_lookahead_radius": ("INT", {"default": 3, "min": 0, "max": 12, "step": 1, "tooltip": "Advanced: Frames davor/danach für Trend-Erkennung."}),
                "body_anchor_trend_tolerance": ("FLOAT", {"default": 8.0, "min": 0.0, "max": 80.0, "step": 1.0, "tooltip": "Advanced: zusätzliche Toleranz für echte Auf/Ab-Bewegung, z.B. Treppen."}),

                "upper_body_anchor_mode": (["neck_shoulders", "shoulders_only", "neck_only"], {"default": "neck_shoulders", "tooltip": "Oberer Referenzpunkt für den Shoulder-Foot-Ratio-Guard."}),
                "shoulder_foot_ratio_guard": ("BOOLEAN", {"default": True, "tooltip": "V22: Soft-Guard gegen zu langen Torso / zu kurze Beine. V19 bleibt Hauptlogik."}),
                "shoulder_foot_ratio_strength": ("FLOAT", {"default": 0.35, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "Wie stark die Hüfte Richtung Zielposition zwischen Schulter und Fuß gezogen wird."}),
                "shoulder_foot_ratio_tolerance": ("FLOAT", {"default": 0.035, "min": 0.0, "max": 0.20, "step": 0.005, "tooltip": "Toleranz für Verhältnisfehler. 0.035 = 3.5 Prozentpunkte."}),
                "shoulder_foot_max_pelvis_shift": ("FLOAT", {"default": 18.0, "min": 0.0, "max": 80.0, "step": 1.0, "tooltip": "Maximale weiche Hüftkorrektur pro Frame."}),
                "shoulder_foot_pelvis_smooth": ("FLOAT", {"default": 0.25, "min": 0.0, "max": 0.95, "step": 0.01, "tooltip": "Glättet die V22-Hüftkorrektur über Frames."}),
                "shoulder_foot_only_fix_short_legs": ("BOOLEAN", {"default": True, "tooltip": "Wenn aktiv, greift V22 nur ein, wenn die Hüfte zu tief liegt und Beine dadurch zu kurz wirken."}),
            },
            "optional": {
                "nlf_render_config": ("STRING", {"forceInput": True, "tooltip": "Die Camera Config aus dem Scaler wird bereinigt."}),
            }
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Retargeting"
    DESCRIPTION = "V22: V19-Core + Soft Shoulder-Foot-Ratio-Guard gegen zu langen Torso / zu kurze Beine."
    ...
    def process(
        ...
        body_anchor_trend_tolerance,
        upper_body_anchor_mode,
        shoulder_foot_ratio_guard,
        shoulder_foot_ratio_strength,
        shoulder_foot_ratio_tolerance,
        shoulder_foot_max_pelvis_shift,
        shoulder_foot_pelvis_smooth,
        shoulder_foot_only_fix_short_legs,
        nlf_render_config="{}"
    ):
        import copy
        import numpy as np
        ...
        if bypass:
            return (
                video_nlf_data,
                "=== NLF PROPORTIONAL RETARGETER V22 ===\nBYPASS AKTIVIERT: Keine Daten verändert.",
                nlf_render_config if nlf_render_config else "{}"
            )

        log_messages = ["=== NLF PROPORTIONAL RETARGETER V22 (V19 CORE + SOFT SHOULDER-FOOT RATIO GUARD) ==="]
        ...
        allowed_upper_modes = ["neck_shoulders", "shoulders_only", "neck_only"]
        if upper_body_anchor_mode not in allowed_upper_modes:
            upper_body_anchor_mode = "neck_shoulders"
        ...
        shoulder_foot_ratio_strength = float(np.clip(shoulder_foot_ratio_strength, 0.0, 1.0))
        shoulder_foot_ratio_tolerance = max(0.0, float(shoulder_foot_ratio_tolerance))
        shoulder_foot_max_pelvis_shift = max(0.0, float(shoulder_foot_max_pelvis_shift))
        shoulder_foot_pelvis_smooth = float(np.clip(shoulder_foot_pelvis_smooth, 0.0, 0.95))
        ...
        log_messages.append(
            f"Shoulder-Foot Ratio Guard: {'AKTIV' if shoulder_foot_ratio_guard and scale_legs else 'INAKTIV'} | "
            f"UpperAnchor: {upper_body_anchor_mode} | "
            f"Strength: {shoulder_foot_ratio_strength:.2f} | "
            f"Tolerance: {shoulder_foot_ratio_tolerance * 100.0:.2f}pp | "
            f"MaxPelvisShift: {shoulder_foot_max_pelvis_shift:.2f} | "
            f"Smooth: {shoulder_foot_pelvis_smooth:.2f} | "
            f"OnlyFixShortLegs: {shoulder_foot_only_fix_short_legs}"
        )
        ...
        def get_upper_body_anchor(p_array):
            ...
            if upper_body_anchor_mode == "neck_only":
                if is_valid_point(p_array, 12):
                    return p_array[12].copy()
        ...
        def get_frame_targets(pts_array):
            if pts_array is None or len(pts_array) <= 12 or not is_valid_point(pts_array, 0) or not is_valid_point(pts_array, 12):
                return {}

            orig_torso_curr = np.linalg.norm(pts_array[12] - pts_array[0]) if np.linalg.norm(pts_array[12]) > 1e-5 else 0.0
            missing_neck_curr = orig_torso_curr * (head_val / 100.0) / 2.0 if is_normalized else head_val / 2.0
            frame_ref_torso = orig_torso_curr + missing_neck_curr
        ...
        def apply_shoulder_foot_ratio_guard(pts_candidate, pts_original, frame_idx, prev_ratio_shift):
            debug = {
                "enabled": shoulder_foot_ratio_guard and scale_legs,
                "applied": False,
                "upper_y": 0.0,
                "foot_y": 0.0,
                "pelvis_y": 0.0,
            ...
            if not shoulder_foot_ratio_guard or not scale_legs or shoulder_foot_ratio_strength <= 0.0:
                return pts_candidate, debug, prev_ratio_shift

            pts_g = pts_candidate.copy()

            upper_anchor = get_upper_body_anchor(pts_g)
            if upper_anchor is None:
                debug["reason"] = "no_upper_anchor"
                return pts_g, debug, prev_ratio_shift

            current_foot_y = get_foot_anchor_y(pts_original, robust=True, percentile=ground_shift_percentile)
            trend_foot_y = foot_y_trend[frame_idx] if frame_idx < len(foot_y_trend) else 0.0

            if ground_anchor_mode == "advanced_trend" and trend_foot_y > 1e-5:
                foot_spike_limit = max(body_anchor_trend_tolerance * 2.0, 12.0)
                if current_foot_y > 1e-5 and abs(current_foot_y - trend_foot_y) > foot_spike_limit:
                    foot_y = trend_foot_y
                else:
                    foot_y = current_foot_y
            else:
                foot_y = current_foot_y

            pelvis_center = get_center(pts_g, [1, 2])
            if pelvis_center is None:
                pelvis_center = get_center(pts_g, [0])

            if pelvis_center is None or foot_y <= 1e-5:
                debug["reason"] = "no_pelvis_or_foot"
                return pts_g, debug, prev_ratio_shift

            upper_y = float(upper_anchor[1])
            pelvis_y = float(pelvis_center[1])

            total_span = foot_y - upper_y
            if abs(total_span) < 1e-5:
                debug["reason"] = "bad_span"
                return pts_g, debug, prev_ratio_shift

            targets = get_frame_targets(pts_g)
            target_ratio, target_torso, target_leg = compute_target_pelvis_ratio(targets)

            current_ratio = (pelvis_y - upper_y) / total_span
            ratio_error = current_ratio - target_ratio

            target_pelvis_y = upper_y + total_span * target_ratio
            raw_shift = target_pelvis_y - pelvis_y

            debug["upper_y"] = upper_y
            debug["foot_y"] = foot_y
            debug["pelvis_y"] = pelvis_y
            debug["target_pelvis_y"] = target_pelvis_y
            debug["target_ratio"] = target_ratio
            debug["current_ratio"] = current_ratio
            debug["ratio_error"] = ratio_error
            debug["raw_shift"] = raw_shift
            debug["target_torso"] = target_torso
            debug["target_leg"] = target_leg

            if abs(ratio_error) <= shoulder_foot_ratio_tolerance:
                debug["reason"] = "within_tolerance"
                return pts_g, debug, prev_ratio_shift

            # Wichtig:
            # Wenn only_fix_short_legs aktiv ist, greifen wir nur ein, wenn die Hüfte zu tief liegt.
            # Y nach unten: current_ratio > target_ratio bedeutet Torso zu lang / Beine zu kurz.
            if shoulder_foot_only_fix_short_legs and ratio_error <= 0.0:
                debug["reason"] = "blocked_not_short_legs"
                return pts_g, debug, prev_ratio_shift

            limited_shift = raw_shift * shoulder_foot_ratio_strength
            if shoulder_foot_max_pelvis_shift > 0.0:
                limited_shift = float(np.clip(limited_shift, -shoulder_foot_max_pelvis_shift, shoulder_foot_max_pelvis_shift))

            smoothed_shift = limited_shift
            if shoulder_foot_pelvis_smooth > 0.0 and prev_ratio_shift is not None:
                smoothed_shift = (prev_ratio_shift * shoulder_foot_pelvis_smooth) + (limited_shift * (1.0 - shoulder_foot_pelvis_smooth))

            final_shift = float(smoothed_shift)

            # V22 macht bewusst KEIN hartes IK und KEIN Foot-Lock.
            # Füße bleiben stehen. Wir verschieben nur Becken/Root und Knie weich.
            # Dadurch kann die Hüfte höher, ohne dass der Ground-Anchor alles wieder wegcancelled.
            pelvis_nodes = [0, 1, 2]
            knee_nodes = [4, 5]

            for idx in pelvis_nodes:
                if is_valid_point(pts_g, idx):
                    pts_g[idx][1] += final_shift

            for idx in knee_nodes:
                if is_valid_point(pts_g, idx):
                    pts_g[idx][1] += final_shift * 0.55

            # Spine weich zwischen neuer Root und altem oberen Körper neu verteilen.
            # Schultern/Kopf bleiben stabiler als bei V21.
            if is_valid_point(pts_g, 0) and is_valid_point(pts_g, 12):
                root_y = float(pts_g[0][1])
                neck_y = float(pts_g[12][1])
                spine_blend = 0.65

                for idx, alpha in [(3, 0.25), (6, 0.50), (9, 0.75)]:
                    if is_valid_point(pts_g, idx):
                        desired_y = root_y * (1.0 - alpha) + neck_y * alpha
                        pts_g[idx][1] = pts_g[idx][1] * (1.0 - spine_blend) + desired_y * spine_blend

            debug["applied"] = True
            debug["limited_shift"] = limited_shift
            debug["smoothed_shift"] = smoothed_shift
            debug["final_shift"] = final_shift
            debug["reason"] = "applied"

            return pts_g, debug, final_shift
        ...
        for frame_idx in range(len(raw_poses)):
            ...
            # --- V22 SOFT SHOULDER-FOOT RATIO GUARD ---
            pts_final, ratio_debug, new_ratio_shift = apply_shoulder_foot_ratio_guard(
                pts_final,
                pts,
                frame_idx,
                prev_ratio_guard_shift
            )

            if ratio_debug.get("applied", False):
                prev_ratio_guard_shift = new_ratio_shift
                ratio_guard_events.append({
                    "frame": frame_idx,
                    "upper_y": ratio_debug["upper_y"],
                    "foot_y": ratio_debug["foot_y"],
                    "pelvis_y": ratio_debug["pelvis_y"],
                    "target_pelvis_y": ratio_debug["target_pelvis_y"],
                    "current_ratio": ratio_debug["current_ratio"],
                    "target_ratio": ratio_debug["target_ratio"],
                    "ratio_error": ratio_debug["ratio_error"],
                    "raw_shift": ratio_debug["raw_shift"],
                    "final_shift": ratio_debug["final_shift"],
                    "reason": ratio_debug["reason"]
                })
            elif shoulder_foot_pelvis_smooth > 0.0 and prev_ratio_guard_shift is not None:
                prev_ratio_guard_shift = prev_ratio_guard_shift * shoulder_foot_pelvis_smooth
            ...
            if log_this_frame:
                ...
                log_messages.append(f"\n--- FRAME {frame_idx} VERGLEICH (V22 V19-Core + Shoulder-Foot Soft Guard) ---")
                log_messages.append(f"Gesamthöhe (Y-Bounding Box) | Vorher: {h_before:.2f} -> Nachher: {h_after:.2f}")
                log_messages.append(f"Schulter-Y Ø             | Vorher: {shoulder_y_before:.2f} -> Nachher: {shoulder_y_after:.2f} | Delta: {(shoulder_y_after - shoulder_y_before):+.2f}")
                log_messages.append(f"Hüft-Y Ø                 | Vorher: {hip_y_before:.2f} -> Nachher: {hip_y_after:.2f} | Delta: {(hip_y_after - hip_y_before):+.2f}")
                log_messages.append(f"Body-Y Ø                 | Vorher: {body_y_before:.2f} -> Nachher: {body_y_after:.2f} | Delta: {(body_y_after - body_y_before):+.2f}")
                ...
                log_messages.append(
                    f"Shoulder-Foot Guard      | Applied: {ratio_debug.get('applied', False)} | Reason: {ratio_debug.get('reason', 'none')} | "
                    f"UpperY: {ratio_debug.get('upper_y', 0.0):.2f} | FootY: {ratio_debug.get('foot_y', 0.0):.2f}"
                )
                log_messages.append(
                    f"Shoulder-Foot Ratio      | Current: {ratio_debug.get('current_ratio', 0.0):.4f} | "
                    f"Target: {ratio_debug.get('target_ratio', 0.0):.4f} | "
                    f"Error: {ratio_debug.get('ratio_error', 0.0) * 100.0:+.2f}pp"
                )
                log_messages.append(
                    f"Shoulder-Foot Pelvis     | PelvisY: {ratio_debug.get('pelvis_y', 0.0):.2f} | "
                    f"TargetPelvisY: {ratio_debug.get('target_pelvis_y', 0.0):.2f} | "
                    f"RawShift: {ratio_debug.get('raw_shift', 0.0):+.2f} | "
                    f"FinalShift: {ratio_debug.get('final_shift', 0.0):+.2f}"
                )
        ...
        # --- SUMMARY: SHOULDER-FOOT GUARD ---
        log_messages.append("\n--- V22 SHOULDER-FOOT RATIO GUARD SUMMARY ---")
        log_messages.append(f"Ratio Guard Events: {len(ratio_guard_events)}")
        log_messages.append(f"Mode: Soft waist correction | Hard IK: False | FootLock: False")

        if ratio_guard_events:
            worst_ratio = max(ratio_guard_events, key=lambda e: abs(e["ratio_error"]))
            strongest_shift = max(ratio_guard_events, key=lambda e: abs(e["final_shift"]))

            log_messages.append(
                f"Stärkster Ratio-Ausreißer: Frame {worst_ratio['frame']} | "
                f"CurrentRatio: {worst_ratio['current_ratio']:.4f} | TargetRatio: {worst_ratio['target_ratio']:.4f} | "
                f"Error: {worst_ratio['ratio_error'] * 100.0:+.2f}pp | Shift: {worst_ratio['final_shift']:+.2f}"
            )
            log_messages.append(
                f"Stärkster Pelvis-Softshift: Frame {strongest_shift['frame']} | "
                f"Pelvis: {strongest_shift['pelvis_y']:.2f} -> Target {strongest_shift['target_pelvis_y']:.2f} | "
                f"RawShift: {strongest_shift['raw_shift']:+.2f} | FinalShift: {strongest_shift['final_shift']:+.2f}"
            )

            log_messages.append("\nErste Ratio-Guard-Events:")
            for e in ratio_guard_events[:25]:
                log_messages.append(
                    f"Frame {str(e['frame']).rjust(4)} | "
                    f"Upper {e['upper_y']:.2f} | Foot {e['foot_y']:.2f} | "
                    f"Pelvis {e['pelvis_y']:.2f}->{e['target_pelvis_y']:.2f} | "
                    f"Ratio {e['current_ratio']:.4f}->{e['target_ratio']:.4f} | "
                    f"Err {e['ratio_error'] * 100.0:+.2f}pp | "
                    f"Shift {e['final_shift']:+.2f}"
                )

            if len(ratio_guard_events) > 25:
                log_messages.append(f"... weitere {len(ratio_guard_events) - 25} Ratio-Guard-Events ausgelassen.")
        else:
            log_messages.append("Keine Shoulder-Foot-Ratio-Eingriffe nötig oder Guard deaktiviert.")

        # --- SUMMARY: LEG GUARD ---
        log_messages.append("\n--- V22 LEG HEIGHT GUARD SUMMARY ---")
        log_messages.append(f"Leg Guard Events: {len(guard_events)}")
        ...
        # --- SUMMARY: GROUND GUARD ---
        log_messages.append("\n--- V22 GROUND ANCHOR GUARD SUMMARY ---")
        log_messages.append(f"Ground Guard Events: {len(ground_guard_events)}")
        log_messages.append(f"Ground Mode benutzt: {ground_anchor_mode}")
