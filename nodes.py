...
class RenderNLFPosesDirectPoseDataMimic:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "width": ("INT", {"default": 512, "min": 64, "max": 4096}),
                "height": ("INT", {"default": 512, "min": 64, "max": 4096}),
                "render_backend": (["taichi", "torch"], {"default": "torch"}),
                "line_thickness": ("FLOAT", {"default": 0.02, "min": 0.001, "max": 0.2, "step": 0.001, "tooltip": "Macht die Zylinder so dünn wie 2D-Linien"}),
                "draw_2d": ("BOOLEAN", {"default": True, "tooltip": "Zeichnet 2D Overlay (falls DW Poses vorhanden)"}),
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/SCAIL"
    DESCRIPTION = "Mimic: Rendert 3D-Daten als 2D-OpenPose-Vektoren. Löst 2D-Verdeckungsprobleme dank Z-Buffer (3D-Tiefe), sieht für ControlNet aber aus wie OpenPose."

    def process(self, nlf_poses, width, height, render_backend="torch", line_thickness=0.02, draw_2d=True, draw_face=True, draw_hands=True, dw_poses_fallback=None, nlf_render_config="{}"):
        import copy
        import json
        import torch
        import numpy as np
        ...
        try:
            ...
            # --- KAMERA CONFIG BAKING (wie V5/V6) ---
            try:
                config = json.loads(nlf_render_config)
                if "anchor_scale" in config:
                    scale_y = float(config["anchor_scale"])
            ...
            # --- MIMIC OPENPOSE MAPPING ---
            # Exakte OpenPose RGB Farben (skaliert auf 0.0 - 1.0 für den 3D Renderer), Alpha = 1.0 (kein Glas-Effekt)
            openpose_colors_rgb = [
                [255, 0, 0],      # 0: Neck -> Nose
                [255, 85, 0],     # 1: R_Sho -> L_Sho (Spezialvektor)
            ...
            # NLF COCO 18 Mapping Indizes.
            # HACK: [2, 5] zeichnet von R_Schulter zu L_Schulter (anstatt 1->2 und 1->5)
            mimic_limb_seq = [
                [1, 0],    # Neck -> Nose
                [2, 5],    # R_Sho -> L_Sho (Spezial Schulter-Hack)
                [2, 3],    # R_Sho -> R_Elb
                [3, 4],    # R_Elb -> R_Wri
                [5, 6],    # L_Sho -> L_Elb
                [6, 7],    # L_Elb -> L_Wri
            ...
            log_messages.append(f"Rendere {len(pose_input)} Frames mit OpenPose-Mimic-Style...")
            
            try:
                # Wir versuchen den Radius zu übergeben, falls der Renderer es unterstützt
                if render_backend == "taichi" and render_whole_taichi is not None:
                    frames_np_rgba = render_whole_taichi(cylinder_specs_list, H=height, W=width, fx=focal_x, fy=focal_y, cx=princpt[0], cy=princpt[1], radius=line_thickness)
                else:
                ...
                log_messages.append("WARNUNG: Renderer ignoriert line_thickness. Nutze Standard.")
                if render_backend == "taichi" and render_whole_taichi is not None:
...
class RenderNLFPosesDirectPoseDataMimic2:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
...
class RenderNLFPosesDirectPoseDataMimic3:
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/SCAIL"
    DESCRIPTION = "Mimic 3: Rendert 3D-Daten als flache 2D-OpenPose-Vektoren inkl. perfekten ViTPose-Farben und Gelenkpunkten."
    ...
    def process(self, nlf_poses, width, height, render_backend="taichi_flat", line_thickness=8.0, draw_2d=True, draw_face=True, draw_hands=True, dw_poses_fallback=None, nlf_render_config="{}"):
        ...
        try:
            ...
            # --- OPENPOSE FARB- UND BONE-MAPPING (ViTPose Exact) ---
            ordered_colors_255 = [
                [255, 85, 0],   # 0 Neck -> R. Shoulder
                [170, 255, 0],  # 1 Neck -> L. Shoulder
                [255, 170, 0],  # 2 R. Shoulder -> R. Elbow
                [255, 255, 0],  # 3 R. Elbow -> R. Wrist
                [85, 255, 0],   # 4 L. Shoulder -> L. Elbow
                [0, 255, 0],    # 5 L. Elbow -> L. Wrist
                [0, 255, 85],   # 6 Neck -> R. Hip
                [0, 255, 170],  # 7 R. Hip -> R. Knee
                [0, 255, 255],  # 8 R. Knee -> R. Ankle
                [0, 170, 255],  # 9 Neck -> L. Hip
                [0, 85, 255],   # 10 L. Hip -> L. Knee
                [0, 0, 255],    # 11 L. Knee -> L. Ankle
                [255, 0, 0],    # 12 Neck -> Nose
                [255, 0, 170],  # 13 Nose -> R. Eye
                [255, 0, 85],   # 14 R. Eye -> R. Ear
            ...
            # Skaliert auf 0.0-1.0 für Taichi/Torch und Alpha = 1.0 (Kein Verblassen)
            ...
            # --- 2. Gelenkpunkte (Circles) exakt wie im Original zeichnen ---
            for i in range(len(frames_np_rgba)):
                frame_img = frames_np_rgba[i]
...
class RenderNLFPosesDirectPoseDataMimic5:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/SCAIL"
    DESCRIPTION = "Mimic 3: Rendert 3D-Daten als 2D-Vektoren für 100% konstante Liniendicke."
                    ...
                    for joints3d in people:
                        ...
                        if np.sum(np.abs(j3d_np)) > 0.01:
                            ...
                            for pt3d in j3d_coco:
                                if np.sum(np.abs(pt3d)) > 0:
                                    pt2d = p3d_single_p2d(pt3d, intrinsic_matrix)
                                    pts_2d.append((int(pt2d[0]), int(pt2d[1])))
                                else:
                                    pts_2d.append(None)
                            all_pts_2d.append(pts_2d)

                    # 2. Knochen zeichnen (Konstante 2D-Dicke!)
                    for pts_2d in all_pts_2d:
                        for limb_idx, limb in enumerate(mimic_limb_seq):
                            start_idx = limb[0]
                            end_idx = limb[1]
                            if pts_2d[start_idx] is not None and pts_2d[end_idx] is not None:
                                color = ordered_colors_255[limb_idx % len(ordered_colors_255)]
                                cv2.line(frame_img, pts_2d[start_idx], pts_2d[end_idx], color, line_thickness, cv2.LINE_AA)

                # 3. Knochen abdunkeln (Alpha 0.6 Effekt aus dem Original)
                frame_img = (frame_img * 0.6).astype(np.uint8)

                # 4. Gelenkpunkte zeichnen (Einstellbare Größe!)
                if pose_input[i] is not None:
                    for pts_2d in all_pts_2d:
                        for j_idx, pt2d in enumerate(pts_2d):
                            if pt2d is not None:
                                x, y = pt2d
                                if 0 <= x < width and 0 <= y < height:
                                    color_rgba = joint_colors_rgb[j_idx % len(joint_colors_rgb)]
                                    cv2.circle(frame_img, (x, y), point_radius, color_rgba, thickness=-1, lineType=cv2.LINE_AA)
...
class RenderNLFPosesDirectPoseDataMimic7:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/SCAIL"
    DESCRIPTION = "Mimic 3: ViTPose Ovale Knochen, korrekte Farben & echtes 3D Z-Sorting."
    ...
    def process(self, nlf_poses, width, height, line_thickness=4, point_radius=4, draw_2d=True, draw_face=True, draw_hands=True, dw_poses_fallback=None, nlf_render_config="{}"):
        ...
        try:
            ...
            # --- EXAKTE OPENPOSE / DWPose FARBEN (Jetzt 100% korrigiert) ---
            # Diese Liste steuert die Knochen (Limbs)
            limb_colors_rgb = [
            ...
            # Diese Liste steuert die Gelenkpunkte (Joints)
            joint_colors_rgb = [
                (255, 0, 0), (255, 85, 0), (255, 170, 0), (255, 255, 0),
                (170, 255, 0), (85, 255, 0), (0, 255, 0), (0, 255, 85),
                (0, 255, 170), (0, 255, 255), (0, 170, 255), (0, 85, 255),
            ...
            for i in range(len(pose_input)):
                ...
                if pose_input[i] is not None:
                    ...
                    # 3. Ovale Knochen zeichnen (Exakte draw_utils.py Logik)
                    for bone in bones_to_draw:
                        x1, y1 = bone['pt1']
                        x2, y2 = bone['pt2']
                        color = bone['color']
                        ...
                        # Dies erzeugt die ovale Form (Verjüngung zu den Gelenken)
                ...
                # 5. Gelenkpunkte ebenfalls nach Z-Tiefe sortieren und voll gesättigt zeichnen
                if pose_input[i] is not None:
                    joints_to_draw = []
                    for pts in all_pts_2d_with_z:
                        for j_idx, pt in enumerate(pts):
            ...
            # --- 6. 2D Details (Gesicht/Hände) drüberlegen ---
            if dw_pose_input is not None and draw_2d:
                canvas_2d = draw_pose_to_canvas_np(dw_pose_input, pool=None, H=height, W=width, reshape_scale=0, show_feet_flag=False, show_body_flag=False, show_cheek_flag=True, dw_hand=True, show_face_flag=draw_face, show_hand_flag=draw_hands)
                for i in range(len(frames_np_rgba)):
                    frame_rgba = frames_np_rgba[i]
...
class RenderNLFPosesDirectPoseDataMimic8:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/SCAIL"
    DESCRIPTION = "Mimic 3: Gerade Schultern, Ovale Knochen, korrekte Farben & 3D Z-Sorting."
                ...
                if pose_input[i] is not None:
                    ...
                    for joints3d in people:
                        ...
                        if np.sum(np.abs(j3d_np)) > 0.01:
                            ...
                            # --- NEU: Gerade Schultern ---
                            # Index 1 = Neck, Index 2 = R.Shoulder, Index 5 = L.Shoulder
                            if len(pts_2d_with_z) > 5 and pts_2d_with_z[2] is not None and pts_2d_with_z[5] is not None:
                                p_r = pts_2d_with_z[2]
                                p_l = pts_2d_with_z[5]
                                
                                # Neuer Hals-Mittelpunkt auf dem unsichtbaren Vektor zwischen den Schultern
...
class RenderNLFPosesDirectPoseDataMimic11:
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/SCAIL"
    DESCRIPTION = "Mimic 10: DWPose Kopf und Gesicht werden exakt auf den NLF Hals verschoben."
    ...
    def process(self, nlf_poses, width, height, line_thickness=4, point_radius=4, head_connection_mode="Offset Head to Neck", draw_2d=True, draw_face=True, draw_hands=True, dw_poses_fallback=None, nlf_render_config="{}"):
        ...
        log_messages = ["=== RENDER NLF POSES POSEDATA MIMIC 10 LOG ==="]
        scaled_nlf_poses = copy.deepcopy(nlf_poses)
                ...
                if pose_input[i] is not None:
                    ...
                    for joints3d in people:
                        ...
                        if np.sum(np.abs(j3d_np)) > 0.01:
                            ...
                            # --- KOMPLETTER HEAD OFFSET (NLF Head Punkte anpassen) ---
                            # Setzt den "roten NLF Punkt" exakt auf den Hals, wie du wolltest
                            if head_connection_mode == "Offset Head to Neck":
                                if pts_2d_with_z[0] is not None and pts_2d_with_z[1] is not None:
                                    offset_x = pts_2d_with_z[1][0] - pts_2d_with_z[0][0]
                                    offset_y = pts_2d_with_z[1][1] - pts_2d_with_z[0][1]
                    ...
                    if dw_pose_input is not None and i < len(dw_pose_input):
                        ...
                        dw_faces = dw_frame.get("faces", [])
                        dw_hands = dw_frame.get("hands", [])
                        dw_bodies = dw_frame.get("bodies", []) # WICHTIG: Bodies laden für den DW-Kopf!

                        for p, pts in enumerate(all_pts_2d_with_z):
                            # --- Hände Alignment (Bleibt unangetastet, funktioniert perfekt) ---
                            if 2*p + 1 < len(dw_hands):
                                l_hand = dw_hands[2*p]     
                            ...
                            # --- Gesicht/Kopf Alignment ---
                            if head_connection_mode == "Offset Head to Neck":
                                if pts[1] is not None: # NLF Hals (Zielposition)
                                    nlf_neck_norm = np.array([pts[1][0] / float(width), pts[1][1] / float(height)])
                                    
                                    offset = None
                                    
                                    # 1. DWPose Nase finden & Offset brechen (NLF Hals - DWPose Nase)
                                    if p < len(dw_bodies):
                                        body = dw_bodies[p]
                                        if len(body) > 0 and body[0][0] > 0: # Nase ist Index 0 in DW Body
                                            dw_nose = np.array([body[0][0], body[0][1]])
                                            offset = nlf_neck_norm - dw_nose
                                            
                                    # Fallback: Gesicht Nasenspitze (Index 30), falls DW Body Nase fehlt
                                    if offset is None and p < len(dw_faces):
                                        face = dw_faces[p]
                                        if len(face) > 30 and face[30][0] > 0:
                                            dw_nose = np.array([face[30][0], face[30][1]])
                                            offset = nlf_neck_norm - dw_nose
                                            
                                    # 2. Offset auf DWPose anwenden
                                    if offset is not None:
                                        # Wende Offset auf DWPose Kopf-Punkte an (0=Nase, 14=RAuge, 15=LAuge, 16=ROhr, 17=LOhr)
                                        if p < len(dw_bodies):
                                            body = dw_bodies[p]
                                            for h_idx in [0, 14, 15, 16, 17]:
                                                if h_idx < len(body) and body[h_idx][0] > 0:
                                                    body[h_idx][0] += offset[0]
                                                    body[h_idx][1] += offset[1]
                                                    
                                        # Wende den exakt gleichen Offset auf alle DWPose Gesichts-Punkte an
                                        if p < len(dw_faces):
                                            face = dw_faces[p]
                                            valid_mask = face[:, 0] > 0
                                            face[valid_mask, 0] += offset[0]  # X-Achse verschieben
                                            face[valid_mask, 1] += offset[1]  # Y-Achse verschieben
                    # ---------------------------------------------------------------------
...
NODE_CLASS_MAPPINGS = {
    ...
    "RenderNLFPosesDirectPoseDataMimic10": RenderNLFPosesDirectPoseDataMimic10,
    "RenderNLFPosesDirectPoseDataMimic11": RenderNLFPosesDirectPoseDataMimic11,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "RenderNLFPosesDirectPoseDataMimic10": "Render NLF Poses Mimic 10 (Flat 3D)",
    "RenderNLFPosesDirectPoseDataMimic11": "Render NLF Poses Mimic 11 (Flat 3D)",
