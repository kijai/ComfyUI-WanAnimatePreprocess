...
# ==============================================================================
# Node: Mask Positional Cutter V6 (Pass Padding Info)
# ==============================================================================

class MaskPositionalCutterV6:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "images": ("IMAGE",),
                "mask": ("MASK",), 
                "padding": ("INT", {"default": 20, "min": 0, "max": 1024, "step": 1, "tooltip": "Initiales Padding (wird später im Joiner evtl. verringert)."}),
                "megapixels": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 16.0, "step": 0.1, "tooltip": "Zielauflösung in MP."}),
                "background_color": (["black", "white"], {"default": "black", "tooltip": "Füllfarbe für den Rand."}),
            },
            "optional": {
    ...
    RETURN_TYPES = ("IMAGE", "MASK", "MASK", "MASK_CUT_INFO_V6", "STRING", "STRING", "STRING", "*", "*", "*")
    RETURN_NAMES = ("cropped_images", "mask_cutted", "mask_cutted_opt", "cut_info", "trans_pos_json", "trans_neg_json", "trans_bbox_json", "trans_pos_raw", "trans_neg_raw", "trans_bbox_raw")
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V6: Bereitet Daten für 'Negative Padding' im Joiner vor."

    def process(self, images, mask, padding, megapixels, background_color, opt_mask=None, opt_positive_points=None, opt_negative_points=None, opt_bboxes=None):
        # Wir nutzen einfach die V5 Logik, verpacken aber das Padding in die Info
        # (Um Codeduplizierung klein zu halten, rufen wir hier quasi V5 Logik ab oder kopieren sie)
        # Hier der vollständige Code für V6 Stand-Alone:
        ...
        for i in range(B):
            ...
            info = {
                ...
                "initial_padding": padding # <--- NEU für V6: Info speichern
            }
            cut_infos.append(info)
...
# ==============================================================================
# Node: Mask Positional Joiner V6 (Negative Padding & Smart Fader)
# ==============================================================================

class MaskPositionalJoinerV6:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "destination_images": ("IMAGE",),
                "processed_images": ("IMAGE",),
                "cut_info": ("MASK_CUT_INFO_V6",), # Muss V6 Info sein (V5 geht zur Not auch)
                "feather": ("INT", {"default": 10, "min": 0, "max": 256, "step": 1, "tooltip": "Weicher Rand."}),
                "padding_minus": ("INT", {"default": 0, "min": 0, "max": 512, "step": 1, "tooltip": "Verringert den Ausschnitt an allen Seiten (Negative Padding). Löst an diesen Kanten automatisch Feathering aus."}),
            },
        }
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V6: Erlaubt das Verkleinern des Einfüge-Bereichs (Minus Padding) mit intelligentem Fading."

    def process(self, destination_images, processed_images, cut_info, feather, padding_minus):
        import cv2
        import numpy as np
        import torch
        ...
        for i in range(B_dest):
            ...
            # Originale Box Infos
            # x1/y1 ist die Position der Box im großen Bild (kann negativ sein wg Padding)
            bbox_x1, bbox_y1, bbox_x2, bbox_y2 = info["bbox"]
            box_w, box_h = info["crop_shape"]
            img_W, img_H = info["original_shape"]
            
            # Border Infos (Wo war das Original zu Ende?)
            pad_l, pad_t, pad_r, pad_b = info.get("padding_borders", (0,0,0,0))
            
            # 1. processed_images zurückskalieren auf die "Cutter Box Größe"
            restored_crop = cv2.resize(proc_img, (int(box_w), int(box_h)), interpolation=cv2.INTER_LANCZOS4)
            
            # 2. PADDING MINUS anwenden (wir schneiden was vom Rand ab)
            # Wir definieren eine "innere Box", die wir tatsächlich nutzen wollen.
            # p_min ist das, was wir wegwerfen.
            p_min = padding_minus
            
            # Koordinaten im "restored_crop" (lokal)
            # Normalerweise 0 bis box_w
            # Mit Minus Padding: p_min bis box_w - p_min
            
            local_x1 = p_min
            local_y1 = p_min
            local_x2 = int(box_w) - p_min
            local_y2 = int(box_h) - p_min
            
            # Falls Padding Minus so groß ist, dass nichts übrig bleibt -> Skip
            if local_x2 <= local_x1 or local_y2 <= local_y1:
                continue

            # 3. Wo landet diese "innere Box" im "destination_image"?
            # bbox_x1 ist der Start der ganzen Box.
            dest_x1 = bbox_x1 + local_x1
            dest_y1 = bbox_y1 + local_y1
            dest_x2 = bbox_x1 + local_x2
            dest_y2 = bbox_y1 + local_y2
            
            # 4. Clipping am Destination Image (was ist wirklich sichtbar?)
            # Wir berechnen die Schnittmenge zwischen "Destination" und "Innerer Box"
            ...
            if visible_dest_x2 <= visible_dest_x1 or visible_dest_y2 <= visible_dest_y1:
                continue
                
            # 5. Entsprechenden Bereich aus "restored_crop" holen
            # Offset berechnen
            ...
            if feather > 0:
                ...
                if f > 0:
                    ...
                    # LOGIK:
                    # Wir faden an einer Kante, WENN:
                    # A) Wir Minus Padding benutzt haben (p_min > 0). 
                    #    Denn dann sind wir "ins Bild gerückt", also ist da Bildmaterial drumherum -> Fade nötig.
                    # B) ODER: Wir hatten KEIN Border Padding vom Cutter (pad == 0).
                    #    Das heißt, die Box war mitten im Bild.
                    
                    # Wir faden NICHT (Hard Cut), WENN:
                    # p_min == 0 UND pad > 0. (Wir sind genau am schwarzen Rand).
                    
                    do_fade_l = (p_min > 0) or (pad_l == 0)
                    do_fade_r = (p_min > 0) or (pad_r == 0)
                    do_fade_t = (p_min > 0) or (pad_t == 0)
                    do_fade_b = (p_min > 0) or (pad_b == 0)
                    
                    # Da wir hier nur den "visible part" haben, müssen wir prüfen, ob wir am Rand des Crops sind.
                    # Aber der Einfachheit halber: Wir faden die Ränder des "to_paste" Blocks,
                    # basierend auf der globalen Logik.
                    
                    # Top
                    if do_fade_t: mask[:f, :] *= grad[:, None]
                    # Bottom
                    if do_fade_b: mask[-f:, :] *= grad[::-1, None]
                    # Left
                    if do_fade_l: mask[:, :f] *= grad[None, :]
                    # Right
                    if do_fade_r: mask[:, -f:] *= grad[None, ::-1]
...
# ==============================================================================
# Node: Mask Positional Cutter V7 (Padding Safety Info)
# ==============================================================================

class MaskPositionalCutterV7:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "images": ("IMAGE",),
                "mask": ("MASK",), 
                "padding": ("INT", {"default": 20, "min": 0, "max": 1024, "step": 1, "tooltip": "Das hier definierte Padding ist das 'Sicherheits-Limit' für den Joiner."}),
                "megapixels": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 16.0, "step": 0.1, "tooltip": "Zielauflösung in MP."}),
                "background_color": (["black", "white"], {"default": "black", "tooltip": "Füllfarbe für den Rand."}),
            },
            "optional": {
    ...
    RETURN_TYPES = ("IMAGE", "MASK", "MASK", "MASK_CUT_INFO_V7", "STRING", "STRING", "STRING", "*", "*", "*")
    RETURN_NAMES = ("cropped_images", "mask_cutted", "mask_cutted_opt", "cut_info", "trans_pos_json", "trans_neg_json", "trans_bbox_json", "trans_pos_raw", "trans_neg_raw", "trans_bbox_raw")
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V7: Gibt Padding-Infos weiter, damit der Joiner niemals in die Maske schneidet."
    ...
    def process(self, images, mask, padding, megapixels, background_color, opt_mask=None, opt_positive_points=None, opt_negative_points=None, opt_bboxes=None):
        ...
        for i in range(B):
            ...
            info = {
                ...
                "original_shape": (W, H),
                "padding_borders": (pad_l, pad_t, pad_r, pad_b),
                "initial_padding": padding # HIER IST DIE WICHTIGE INFO
            }
            cut_infos.append(info)
...
# ==============================================================================
# Node: Mask Positional Joiner V7 (Clamp Minus Padding to Safe Zone)
# ==============================================================================

class MaskPositionalJoinerV7:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "destination_images": ("IMAGE",),
                "processed_images": ("IMAGE",),
                "cut_info": ("MASK_CUT_INFO_V7",), # Braucht V6/V7 Info
                "feather": ("INT", {"default": 10, "min": 0, "max": 256, "step": 1, "tooltip": "Weicher Rand."}),
                "padding_minus": ("INT", {"default": 0, "min": 0, "max": 512, "step": 1, "tooltip": "Schneidet Ränder ab. WICHTIG: Stoppt automatisch am Masken-Rand (initial_padding)."}),
            },
        }
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V7: Sicherer Joiner. Minus-Padding kann niemals in die Maske schneiden (wird automatisch auf initial_padding begrenzt)."
    ...
    def process(self, destination_images, processed_images, cut_info, feather, padding_minus):
        ...
        for i in range(B_dest):
            ...
            # --- SICHERHEITS-LOGIK V7 ---
            # Wir holen uns das ursprüngliche Padding aus dem Cutter.
            # Falls es fehlt (alte Nodes), nehmen wir einen großen Wert (kein Schutz).
            init_pad = info.get("initial_padding", 99999)
            
            # Das ist der entscheidende Check:
            # Wir nehmen das Minimum. Wenn User 500 eingibt, aber Pad nur 20 war, nehmen wir 20.
            p_min = min(padding_minus, init_pad)
            ...
            # 2. Local Crop (mit Safe Minus Padding)
            local_x1 = p_min
            local_y1 = p_min
            local_x2 = int(box_w) - p_min
            local_y2 = int(box_h) - p_min
            ...
            # 6. Feathering (angepasst an Safe Limit)
            if feather > 0:
                h_p, w_p = to_paste.shape[:2]
                mask = np.ones((h_p, w_p), dtype=np.float32)
                f = min(feather, w_p//2, h_p//2)
                ...
                if f > 0:
                    ...
                    # Wir faden nur, wenn wir tatsächlich "ins Bild" geschnitten haben (p_min > 0)
                    # ODER wenn wir mitten im Bild sind (pad == 0).
                    # Da p_min jetzt "Safe" ist, können wir sicher sein, dass wir maximal bis zur Maske faden.
...
# ==============================================================================
# Node: Mask Positional Cutter V8
# ==============================================================================

class MaskPositionalCutterV8:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    RETURN_TYPES = ("IMAGE", "MASK", "MASK", "MASK_CUT_INFO_V8", "STRING", "STRING", "STRING", "*", "*", "*")
    RETURN_NAMES = ("cropped_images", "mask_cutted", "mask_cutted_opt", "cut_info", "trans_pos_json", "trans_neg_json", "trans_bbox_json", "trans_pos_raw", "trans_neg_raw", "trans_bbox_raw")
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V8: Cutter für den V8 Joiner. Verbinde 'mask_cutted' mit dem Joiner!"
...
# ==============================================================================
# Node: Mask Positional Joiner V8 (Smart Mask Protect & Screen Edge Detection)
# ==============================================================================

class MaskPositionalJoinerV8:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "destination_images": ("IMAGE",),
                "processed_images": ("IMAGE",),
                "cut_info": ("MASK_CUT_INFO_V8",),
                "feather": ("INT", {"default": 10, "min": 0, "max": 256, "step": 1, "tooltip": "Weicher Rand."}),
                "padding_minus": ("INT", {"default": 0, "min": 0, "max": 1024, "step": 1, "tooltip": "Schneidet Ränder ab. Stoppt automatisch vor der Maske, falls angeschlossen."}),
            },
            "optional": {
                "opt_mask_cutted": ("MASK",), # HIER die mask_cutted vom Cutter anschließen!
            }
        }
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V8: Nutzt die gecuttete Maske als Schutzschild. Kein Padding/Feather in die Maske! Perfekte Bildranderkennung."
    ...
    def process(self, destination_images, processed_images, cut_info, feather, padding_minus, opt_mask_cutted=None):
        ...
        for i in range(B_dest):
            ...
            # 5. MATHEMATISCH PERFEKTE Bildranderkennung
            # Wir clippen die Koordinaten exakt an die Ränder des Originalbildes
            visible_dest_x1 = max(0, dest_x1)
            visible_dest_y1 = max(0, dest_y1)
            visible_dest_x2 = min(img_W, dest_x2)
            visible_dest_y2 = min(img_H, dest_y2)
            ...
            # Offset für den Ausschnitt aus dem lokalen Bild
            offset_x = visible_dest_x1 - dest_x1
            offset_y = visible_dest_y1 - dest_y1
            ...
            if feather > 0:
                ...
                # Wir setzen das Fade-Limit. Es darf nicht größer als die halbe Box sein.
                f_l = min(feather, w_p//2)
                f_r = min(feather, w_p//2)
                f_t = min(feather, h_p//2)
                f_b = min(feather, h_p//2)
                
                # HIER IST DIE MAGIE: Wir faden NUR, wenn der Rand INNERHALB des Bildes liegt!
                # Wenn visible_dest_x1 == 0 ist, kleben wir am Bildschirmrand. -> KEIN Fade.
                ...
                # MASKEN-SCHUTZSCHILD: Überschreibe den Fader dort, wo die Maske der Person ist!
                if restored_mask is not None:
                    mask_slice = restored_mask[final_src_y1:final_src_y2, final_src_x1:final_src_x2]
                    # np.maximum behält die "1.0" der Maske bei, selbst wenn der Fader dort "0.2" machen will!
...
# ==============================================================================
# Node: Mask Positional Cutter V10 (Sub-Pixel Accuracy & Micro-Jitter Fix)
# ==============================================================================

class MaskPositionalCutterV10:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "padding": ("INT", {"default": 20, "min": 0, "max": 1024, "step": 1, "tooltip": "Sicherheitsrand um die Person."}),
                "megapixels": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 16.0, "step": 0.1, "tooltip": "Zielauflösung in MP."}),
                "camera_smooth_window": ("INT", {"default": 10, "min": 0, "max": 100, "step": 1, "tooltip": "Glättet das Wackeln (0 = Aus, Höher = Smoother)."}),
                "keep_mask_100_percent_inside": (["yes", "no"], {"default": "yes", "tooltip": "Erzwingt, dass keine weißen Stellen abgeschnitten werden."}),
                "background_color": (["black", "white"], {"default": "black", "tooltip": "Füllfarbe am Bildrand."}),
            },
            "optional": {
                "opt_mask": ("MASK",),
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V10: Nutzt Sub-Pixel Affine Transforms, um Mikroruckler komplett zu eliminieren."
    ...
    def process(self, images, mask, padding, megapixels, camera_smooth_window, keep_mask_100_percent_inside, background_color, opt_mask=None, opt_positive_points=None, opt_negative_points=None, opt_bboxes=None):
        ...
        # --- 2. CAMERA & BOUNDS SMOOTHING (Gegen Mikroruckler) ---
        smoothed_centers = []
        smoothed_bounds = []
        ...
        if camera_smooth_window > 0:
            ...
            for i in range(B):
                ...
                # Wir glätten auch die Masken-Wände gegen AI-Jitter
                valid_bounds = [b for b in mask_bounds[start:end] if b is not None]
                if len(valid_bounds) > 0:
                    avg_x1 = sum(b[0] for b in valid_bounds) / len(valid_bounds)
                    avg_y1 = sum(b[1] for b in valid_bounds) / len(valid_bounds)
                    avg_x2 = sum(b[2] for b in valid_bounds) / len(valid_bounds)
                    avg_y2 = sum(b[3] for b in valid_bounds) / len(valid_bounds)
                    smoothed_bounds.append((avg_x1, avg_y1, avg_x2, avg_y2))
                else:
                    smoothed_bounds.append(None)
        else:
            smoothed_centers = valid_centers.copy()
            smoothed_bounds = mask_bounds.copy()

        # --- 3. STRICT MASK CONTAINMENT ---
        if keep_mask_100_percent_inside == "yes":
            for i in range(B):
                if smoothed_bounds[i] is not None:
        ...
        # Sub-Pixel Transformations-Funktion für Punkte
        ...
        for i in range(B):
            ...
            # Sub-Pixel Verschiebungs-Matrix für cv2.warpAffine
            # t_x = Ziel_Zentrum - (Quell_Zentrum * Skalierung)
            t_x = (target_w / 2.0) - (cx * scale_x)
            t_y = (target_h / 2.0) - (cy * scale_y)
            
            M = np.array([[scale_x, 0, t_x], [0, scale_y, t_y]], dtype=np.float64)
            
            # --- NEU IN V10: Wunderschöner Sub-Pixel Warp ---
            # Statt das Bild hart zu schneiden, rechnet warpAffine Kommastellen weich um.
            ...
            # Joiner braucht weiterhin Integer für das Einfügen (die Rück-Zitter-Wirkung ist durch statischen HG unsichtbar)
            int_x1 = int(round(cx - box_w / 2.0))
            int_y1 = int(round(cy - box_h / 2.0))
            int_x2 = int_x1 + box_w
            int_y2 = int_y1 + box_h
            
            pad_l, pad_t = max(0, -int_x1), max(0, -int_y1)
            pad_r, pad_b = max(0, int_x2 - W), max(0, int_y2 - H)

            info = {
                "bbox": (int_x1, int_y1, int_x2, int_y2),
                "crop_shape": (box_w, box_h),
                "target_shape": (target_w, target_h), 
                "original_shape": (W, H),
                "padding_borders": (pad_l, pad_t, pad_r, pad_b),
...
# ==============================================================================
# Node: Mask Positional Cutter V11 (Multi-Pass Gimbal Smooth)
# ==============================================================================

class MaskPositionalCutterV11:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "images": ("IMAGE",),
                "mask": ("MASK",), 
                "padding": ("INT", {"default": 30, "min": 0, "max": 1024, "step": 1, "tooltip": "Sicherheitsrand. Wichtig als Puffer für weiches Smoothing!"}),
                "megapixels": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 16.0, "step": 0.1, "tooltip": "Zielauflösung in MP."}),
                "camera_smooth_window": ("INT", {"default": 10, "min": 0, "max": 100, "step": 1, "tooltip": "Wie viele Frames vor/zurück beachtet werden."}),
                "smoothing_passes": ("INT", {"default": 3, "min": 1, "max": 10, "step": 1, "tooltip": "NEU: Wie oft die Glättung angewendet wird. 3-5 eliminiert Mikroruckler (Gauß-Effekt)."}),
                "keep_mask_100_percent_inside": (["yes", "no"], {"default": "yes", "tooltip": "Verhindert, dass die Person abgeschnitten wird."}),
                "background_color": (["black", "white"], {"default": "black", "tooltip": "Füllfarbe am Bildrand."}),
            },
            "optional": {
                "opt_mask": ("MASK",),
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V11: Multi-Pass Smoothing und vorberechnetes Clamping für absolut perfekte, mikrorucklerfreie Gimbal-Kamerafahrten."
    ...
    def process(self, images, mask, padding, megapixels, camera_smooth_window, smoothing_passes, keep_mask_100_percent_inside, background_color, opt_mask=None, opt_positive_points=None, opt_negative_points=None, opt_bboxes=None):
        ...
        # --- 2. PRE-CLAMPING (Die harte Masken-Garantie ZUERST) ---
        # Wir zwingen die unruhigen Center in die Masken-Box, bevor wir glätten!
        pre_smoothed_centers = []
        for i in range(B):
            cx, cy = valid_centers[i]
            if keep_mask_100_percent_inside == "yes" and mask_bounds[i] is not None:
                x1, y1, x2, y2 = mask_bounds[i]
                
                # Wie viel darf der Center wackeln, ohne dass die Maske den Rahmen verlässt?
                min_cx = x2 - box_w / 2.0
                max_cx = x1 + box_w / 2.0
                min_cy = y2 - box_h / 2.0
                max_cy = y1 + box_h / 2.0
            ...
            pre_smoothed_centers.append((cx, cy))

        # --- 3. MULTI-PASS SMOOTHING (Der Gimbal-Gauß-Effekt gegen Mikroruckler) ---
        # Jetzt machen wir die Route absolut butterweich.
        smoothed_centers = pre_smoothed_centers.copy()
        if camera_smooth_window > 0:
            pad_w = camera_smooth_window
            for _ in range(smoothing_passes):
                ...
                for i in range(B):
                    ...
                    window = smoothed_centers[start:end]
                    avg_cx = sum(c[0] for c in window) / len(window)
                    avg_cy = sum(c[1] for c in window) / len(window)
                    temp_centers.append((avg_cx, avg_cy))
                smoothed_centers = temp_centers
...
# ==============================================================================
# Node: Mask Positional Cutter V12 (Gimbal Smooth & Sub-Pixel Engine)
# ==============================================================================

class MaskPositionalCutterV12:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "megapixels": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 16.0, "step": 0.1, "tooltip": "Zielauflösung in MP."}),
                "camera_smooth_window": ("INT", {"default": 10, "min": 0, "max": 100, "step": 1, "tooltip": "Gimbal Window (0 = Aus, Höher = Smoother)."}),
                "smoothing_passes": ("INT", {"default": 3, "min": 1, "max": 10, "step": 1, "tooltip": "Multi-Pass für butterweiche Kamera ohne Mikroruckler."}),
    ...
    RETURN_TYPES = ("IMAGE", "MASK", "MASK", "MASK_CUT_INFO_V12", "STRING", "STRING", "STRING", "*", "*", "*")
    RETURN_NAMES = ("cropped_images", "mask_cutted", "mask_cutted_opt", "cut_info", "trans_pos_json", "trans_neg_json", "trans_bbox_json", "trans_pos_raw", "trans_neg_raw", "trans_bbox_raw")
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V12: Multi-Pass Smoother. Speichert exakte Sub-Pixel Float-Koordinaten für den V12 Joiner."
    ...
    def process(self, images, mask, padding, megapixels, camera_smooth_window, smoothing_passes, keep_mask_100_percent_inside, background_color, opt_mask=None, opt_positive_points=None, opt_negative_points=None, opt_bboxes=None):
        ...
        # Fehlende KI-Frames überbrücken
        valid_centers = []
        last_valid = (W/2.0, H/2.0)
        for c in raw_centers:
            if c is not None: last_valid = c
        ...
        # Kamera weich aber STRIKT auf die glatten Maskenränder klammern
        final_centers = []
        for i in range(B):
            cx, cy = smoothed_centers[i]
            if keep_mask_100_percent_inside == "yes":
        ...
        for i in range(B):
            ...
            # V12 WICHTIG: Wir übergeben die Float-Center cx/cy an den Joiner!
            info = {
                "cx": float(cx),
                "cy": float(cy),
                "crop_shape": (box_w, box_h),
...
# ==============================================================================
# Node: Mask Positional Joiner V12 (Inverse Sub-Pixel Matrix)
# ==============================================================================

class MaskPositionalJoinerV12:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V12: Nutzt OpenCV Affine Inverse. Eliminiert Mikroruckler komplett. Schützt die Maske."
    ...
    def process(self, destination_images, processed_images, cut_info, feather, padding_minus, opt_mask_cutted=None):
        ...
        for i in range(B_dest):
            ...
            # Sub-Pixel Float Koordinaten aus V12 Cutter
            cx, cy = info["cx"], info["cy"]
            box_w, box_h = info["crop_shape"]
            target_w, target_h = info["target_shape"]
            img_W, img_H = info["original_shape"]
            
            scale_x = float(target_w) / float(box_w)
            scale_y = float(target_h) / float(box_h)
            
            # --- 1. Inverse Sub-Pixel Matrix berechnen ---
            # Fügt das Bild EXAKT auf die Nachkommastelle genau dort ein, wo es herkam.
            M_inv = np.array([
                [1.0 / scale_x, 0, cx - box_w / 2.0],
                [0, 1.0 / scale_y, cy - box_h / 2.0]
            ], dtype=np.float64)
            
            # --- 2. Alpha Maske (Padding Minus & Feather) im Target-Space generieren ---
            pm_x = padding_minus * scale_x
            pm_y = padding_minus * scale_y
            ...
            if opt_mask_cutted is not None:
                ...
                if len(y_ind) > 0:
                    m_x1, m_x2 = x_ind.min(), x_ind.max()
                    m_y1, m_y2 = y_ind.min(), y_ind.max()
                    # Masken-Panzer: Minus Padding darf Maske nicht verletzen
                    safe_pm_l = min(pm_x, m_x1)
                    safe_pm_r = min(pm_x, target_w - m_x2)
                    safe_pm_t = min(pm_y, m_y1)
                    safe_pm_b = min(pm_y, target_h - m_y2)
                else:
                    safe_pm_l = safe_pm_r = safe_pm_t = safe_pm_b = 0
            
            alpha = np.zeros((target_h, target_w), dtype=np.float32)
            
            start_x = int(math.floor(safe_pm_l))
            end_x = int(math.ceil(target_w - safe_pm_r))
            start_y = int(math.floor(safe_pm_t))
            end_y = int(math.ceil(target_h - safe_pm_b))
            
            if end_x > start_x and end_y > start_y:
                alpha[start_y:end_y, start_x:end_x] = 1.0
                
                if feather > 0:
                    f_x = int(feather * scale_x)
                    f_y = int(feather * scale_y)
                    
                    # Bildränder erkennen (Wurde die Original-Kamera am Rand abgeschnitten?)
                    dist_l = cx - box_w/2.0
                    dist_r = img_W - (cx + box_w/2.0)
                    dist_t = cy - box_h/2.0
                    dist_b = img_H - (cy + box_h/2.0)
                    
                    # Wir faden nur, wenn wir nicht am harten Rand des Originalvideos kleben
                    do_fade_l = (dist_l > 0.5) or (start_x > 0)
                    do_fade_r = (dist_r > 0.5) or (end_x < target_w)
                    do_fade_t = (dist_t > 0.5) or (start_y > 0)
                    do_fade_b = (dist_b > 0.5) or (end_y < target_h)
            ...
            # Masken-Schutzschild anwenden (Person hat immer Alpha=1)
            if msk_img is not None:
                alpha = np.maximum(alpha, msk_img)
            
            # --- 3. THE MAGIC: Alles zurück in den großen Hintergrund warpen ---
            # Statt Arrays aufwendig zu zerschneiden, macht die Matrix das automatisch und fehlerfrei.
            warped_proc = cv2.warpAffine(proc_img, M_inv, (img_W, img_H), flags=cv2.INTER_LANCZOS4, borderMode=cv2.BORDER_CONSTANT, borderValue=(0,0,0))
            warped_alpha = cv2.warpAffine(alpha, M_inv, (img_W, img_H), flags=cv2.INTER_LANCZOS4, borderMode=cv2.BORDER_CONSTANT, borderValue=0.0)
...
# ==============================================================================
# Node: Mask Positional Cutter V13
# ==============================================================================

class MaskPositionalCutterV13:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    RETURN_TYPES = ("IMAGE", "MASK", "MASK", "MASK_CUT_INFO_V13", "STRING", "STRING", "STRING", "*", "*", "*")
    RETURN_NAMES = ("cropped_images", "mask_cutted", "mask_cutted_opt", "cut_info", "trans_pos_json", "trans_neg_json", "trans_bbox_json", "trans_pos_raw", "trans_neg_raw", "trans_bbox_raw")
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V13: Sub-Pixel Cutter für Auto-Resize Joiner V13."
...
# ==============================================================================
# Node: Mask Positional Joiner V13 (Auto-Resize & Size Agnostic)
# ==============================================================================

class MaskPositionalJoinerV13:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V13: Ist komplett immun gegen Resize-Nodes. Skaliert die Maske automatisch auf die Video-Größe."
    ...
    def process(self, destination_images, processed_images, cut_info, feather, padding_minus, opt_mask_cutted=None):
        ...
        for i in range(B_dest):
            ...
            # Die TATSÄCHLICHE Größe des eintreffenden Videos auslesen!
            curr_h, curr_w = proc_img.shape[:2]
            ...
            # Skalierungsfaktor on-the-fly neu berechnen
            scale_x = float(curr_w) / float(box_w)
            scale_y = float(curr_h) / float(box_h)
            
            # --- 1. Inverse Sub-Pixel Matrix ---
            M_inv = np.array([
                [1.0 / scale_x, 0, cx - box_w / 2.0],
                [0, 1.0 / scale_y, cy - box_h / 2.0]
            ], dtype=np.float64)
            
            # --- 2. Alpha Maske im aktuellen Space generieren ---
            pm_x = padding_minus * scale_x
            pm_y = padding_minus * scale_y
            ...
            if opt_mask_cutted is not None:
                ...
                # Wenn die Maske eine andere Auflösung hat als das aktuelle Video, 
                # skaliert der Joiner sie jetzt automatisch passend!
...
# ==============================================================================
# Node: Mask Positional Joiner V14 (Total Size Agnostic)
# ==============================================================================

class MaskPositionalJoinerV14:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V14: Immun gegen Resize-Nodes bei Hintergrund- UND Vordergrundbildern. Adaptiert Koordinaten dynamisch in Echtzeit."
    ...
    def process(self, destination_images, processed_images, cut_info, feather, padding_minus, opt_mask_cutted=None):
        ...
        for i in range(B_dest):
            ...
            # 1. Die TATSÄCHLICHE Größe der ankommenden Bilder auslesen!
            curr_h, curr_w = proc_img.shape[:2]       # Das Wan-Video
            dest_h, dest_w = dest_np[i].shape[:2]     # Das Hintergrund-Bild
            
            # Original-Daten vom Cutter
            cx, cy = info["cx"], info["cy"]
            box_w, box_h = info["crop_shape"]
            img_W, img_H = info["original_shape"]
            
            # --- THE MAGIC V14: Dynamische Hintergrund-Skalierung ---
            # Wenn der Hintergrund zwischen Cutter und Joiner resizt wurde,
            # passen wir die Cutter-Box-Koordinaten an das neue Format an:
            ...
            # --- Dynamische Vordergrund-Skalierung ---
            # Wie muss das Wan-Video skaliert werden, um in die neue Box zu passen?
            fg_scale_x = float(curr_w) / box_w_bg
            fg_scale_y = float(curr_h) / box_h_bg
            
            # Inverse Sub-Pixel Matrix (Fügt Vordergrund in den NEUEN Hintergrund ein)
            M_inv = np.array([
                [1.0 / fg_scale_x, 0, cx_bg - box_w_bg / 2.0],
                [0, 1.0 / fg_scale_y, cy_bg - box_h_bg / 2.0]
            ], dtype=np.float64)
            
            # --- 2. Alpha Maske im aktuellen Foreground Space generieren ---
            # padding_minus basierte auf der Original-Box. Wir rechnen es auf den curr_w Space um:
            ...
            if opt_mask_cutted is not None:
                ...
                # Wenn Maske andere Auflösung hat, schnell passend ziehen
                if raw_msk.shape[:2] != (curr_h, curr_w):
                    msk_img = cv2.resize(raw_msk, (curr_w, curr_h), interpolation=cv2.INTER_LINEAR)
                else:
                    msk_img = raw_msk
            ...
            if end_x > start_x and end_y > start_y:
                ...
                if feather > 0:
                    ...
                    # Abstand zum Rand des (neuen) Hintergrunds
            ...
            # --- 3. Warpen in den neuen Hintergrund ---
...
# ==============================================================================
# Node: Wan Smart Image Batcher (Ersetzt die normale ImageBatch Node im Loop)
# ==============================================================================

class WanSmartImageBatcher:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "image1": ("IMAGE", {"tooltip": "Der akkumulierte Video-Batch (z.B. initial_value1 vom Loop, also dein First-Pass)."}),
                "image2": ("IMAGE", {"tooltip": "Der neu generierte Video-Chunk aus WanAnimate (innerhalb des Loops)."}),
                "sync_settings": ("FRAME_SYNC_SETTINGS", {"tooltip": "Verbinde hier die Wan Frame Sync Settings V4."}),
            },
            "optional": {
                "opt_mask1": ("MASK",),
                "opt_mask2": ("MASK",),
    ...
    DESCRIPTION = "Intelligenter Batcher: Repariert Längen und schneidet Overlap ab, bevor die Chunks zusammengeklebt werden."
    ...
    def process(self, image1, image2, sync_settings, opt_mask1=None, opt_mask2=None):
        ...
        # --- 1. FIRST PASS (image1) FIXEN ---
        # Das machen wir NUR beim allerersten Durchgang (Iteration 0)!
        # Danach ist image1 bereits fertig akkumuliert und darf nicht mehr verändert werden.
        if iteration == 0 and target == "Foreground (Wan Output)" and exp_len_first > 0:
            list1 = fix_list(list1, exp_len_first)
            if mlist1 is not None:
                mlist1 = fix_list(mlist1, exp_len_first)

        # --- 2. LOOP PASS (image2) FIXEN ---
        # A) Zuerst den Overlap gnadenlos am Anfang abschneiden
        if overlap_drop > 0:
        ...
        # B) Dann die Länge korrigieren (Falls er zu wenig/zu viel generiert hat)
        if target == "Foreground (Wan Output)" and exp_len_loop > 0:
            # Die Ziellänge ist das, was NACH dem Overlap-Drop übrig bleiben soll!
            # Bsp: Wenn du 16 Frames willst und 10 droppst, müssen 6 übrig bleiben.
            target_len2 = max(1, exp_len_loop - overlap_drop)
            list2 = fix_list(list2, target_len2)
            if mlist2 is not None:
                mlist2 = fix_list(mlist2, target_len2)

        # --- 3. BATCHEN (Zusammenkleben) ---
        out_img_list = list1 + list2
        out_img = torch.stack(out_img_list)

        # Masken sicher batchen (falls mal eine fehlt, Dummy erstellen)
...
# ==============================================================================
# Node: Mask Positional Joiner V19 (Final Out-of-Loop Joiner)
# ==============================================================================

class MaskPositionalJoinerV19:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "destination_images": ("IMAGE",),
                "processed_images": ("IMAGE",),
                "cut_info": ("MASK_CUT_INFO_V14",), # Akzeptiert weiterhin V14 Info
                "feather": ("INT", {"default": 10, "min": 0, "max": 256, "step": 1}),
                "padding_minus": ("INT", {"default": 0, "min": 0, "max": 1024, "step": 1}),
            },
            "optional": {
                "opt_mask_cutted": ("MASK",),
                "sync_settings": ("FRAME_SYNC_SETTINGS",), # HIER auch die V4 Settings anstecken
            }
        }
    ...
    FUNCTION = "process"
    CATEGORY = "WanAnimatePreprocess/Masking"
    DESCRIPTION = "V19: Gehört NACH den Loop. Agnostisch zu Resizing, führt finalen Längen-Sync durch."
    ...
    def process(self, destination_images, processed_images, cut_info, feather, padding_minus, opt_mask_cutted=None, sync_settings=None):
        ...
        # --- 1. FINALER GESAMT-LÄNGEN SYNC ---
        # Falls die Längen vom großen Background und dem fertigen Batched-Video am Ende
        # noch abweichen sollten (z.B. weil du Background Target gewählt hast), 
        # repariert der Joiner das hier in einem Rutsch.
        if sync_settings and len(dest_list) != len(proc_list):
            target_name = sync_settings["target"]
            idx_from_end = sync_settings["idx_from_end"]
            
            if target_name == "Foreground (Wan Output)":
                t_len = len(dest_list)
                while len(proc_list) > t_len:
                    idx = max(0, len(proc_list) - 1 - idx_from_end)
                    proc_list.pop(idx)
                    if mask_list is not None: mask_list.pop(idx)
                while len(proc_list) < t_len:
                    idx = max(0, len(proc_list) - 1 - idx_from_end)
                    proc_list.insert(idx, proc_list[idx].clone())
                    if mask_list is not None: mask_list.insert(idx, mask_list[idx].clone())
            else:
                # Target ist Background -> Background wird eingekürzt/verlängert
                t_len = len(proc_list)
                while len(dest_list) > t_len:
                    idx = max(0, len(dest_list) - 1 - idx_from_end)
                    dest_list.pop(idx)
                    cut_list.pop(idx)
                while len(dest_list) < t_len:
                    idx = max(0, len(dest_list) - 1 - idx_from_end)
                    dest_list.insert(idx, dest_list[idx].clone())
                    cut_list.insert(idx, cut_list[idx].copy())
...
NODE_CLASS_MAPPINGS = {
    ...
    "MaskPositionalCutterV6": MaskPositionalCutterV6,
    "MaskPositionalJoinerV6": MaskPositionalJoinerV6,
    "MaskPositionalCutterV7": MaskPositionalCutterV7,
    ...
    "MaskPositionalCutterV10": MaskPositionalCutterV10,
    "MaskPositionalCutterV11": MaskPositionalCutterV11,
    "MaskPositionalCutterV12": MaskPositionalCutterV12,
    "MaskPositionalJoinerV12": MaskPositionalJoinerV12,
    "MaskPositionalCutterV13": MaskPositionalCutterV13,
    ...
    "WanSmartImageBatcher": WanSmartImageBatcher,
    "MaskPositionalJoinerV19": MaskPositionalJoinerV19,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "MaskPositionalCutterV6": "Mask Positional Cutter V6 (Negative Padding Prep)",
    "MaskPositionalJoinerV6": "Mask Positional Joiner V6 (Negative Padding)",
    "MaskPositionalCutterV7": "Mask Positional Cutter V7 (Safe Padding Info)",
    ...
    "MaskPositionalCutterV10": "Mask Positional Cutter V10 (Sub-Pixel Anti-Jitter)",
    "MaskPositionalCutterV11": "Mask Positional Cutter V11 (Ultimate Anti-Jitter)",
    "MaskPositionalCutterV12": "Mask Positional Cutter V12 (Gimbal & Matrix)",
    "MaskPositionalJoinerV12": "Mask Positional Joiner V12 (Inverse Matrix Paste)",
    "MaskPositionalCutterV13": "Mask Positional Cutter V13",
    ...
    "WanSmartImageBatcher": "Wan Smart Image Batcher",
    "MaskPositionalJoinerV19": "Mask Positional Joiner V19",
