...
class PoseGlobalPerspectiveScalerV17:
    ...
    def process(self, video_pose_data, calibration_data, video_depth_map, include_head, anchor_window, min_confidence, video_nlf_data=None):
        ...
        if not pose_metas:
            return (pose_data_copy, "Fehler: Keine Pose-Daten.")
        ...
        for meta in pose_metas:
            ...
            body_lengths.append(max(valid_y) - min(valid_y) if valid_y else 0.0)
        ...
        for i in range(start_idx, end_idx + 1):
            ...
            norm = get_torso_norm(kps)
            if norm == 0.0: continue
            valid_x, valid_y = [kps[1][0], kps[8][0], kps[11][0]], [kps[1][1], kps[8][1], kps[11][1]]
            min_x, max_x = int(max(0, min(valid_x))), int(min(W-1, max(valid_x)))
            min_y, max_y = int(max(0, min(valid_y))), int(min(H-1, max(valid_y)))
            frame_depth = float(np.mean(depth_np[min(i, H-1), min_y:max_y, min_x:max_x])) if max_x > min_x and max_y > min_y else 0.5
            if is_inverted: frame_depth = 1.0 / max(frame_depth, 0.0001)
            sum_norm += norm
            sum_depth += frame_depth
            valid_frames_in_window += 1
...
# ======================================================================
# 3. V18: GLOBAL PERSPECTIVE SCALER (FIXED CONFIDENCE & SCORING)
# ======================================================================
class PoseGlobalPerspectiveScalerV18:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    def process(self, video_pose_data, calibration_data, video_depth_map, include_head, anchor_window, min_confidence, video_nlf_data=None):
        ...
        pose_data_copy = copy.deepcopy(video_pose_data)
        pose_metas = pose_data_copy.get("pose_metas", [])
        log_messages = ["=== V18 GLOBAL SCALER LOG (SCORING) ==="]
        ...
        # FIX: Liest die Confidence korrekt aus dem separaten Array
        ...
        # FIX: Arbeitet jetzt korrekt mit kps_body [X, Y] und kps_body_p [Conf]
        ...
        # 1. Maximale Körperlänge berechnen
        for meta in pose_metas:
            kps_2d = getattr(meta, "kps_body", None)
        ...
        for i, meta in enumerate(pose_metas):
            ...
            # Ein Torso muss vorhanden sein, ansonsten -1
            if kps_2d is None or len(kps_2d) < 14 or get_torso_norm(kps_2d, confs) == 0.0:
                frame_scores.append(-1.0)
                score_details.append({"w":0, "f":0, "l":0, "t":-1})
            ...
            # --- Füße / Hacken (Indizes 18, 19, 20 und 21, 22, 23) ---
            has_feet = False
            feet_indices = [18, 19, 20, 21, 22, 23]
            for idx in feet_indices:
                if is_valid_point(kps_2d, confs, idx):
                    has_feet = True
                    found_points.append(f"Fuß({idx})[{get_conf(kps_2d, confs, idx):.2f}]")
            
            # --- Knöchel / Waden (Indizes 10, 13) ---
            has_ankles = False
            for idx in [10, 13]:
                if is_valid_point(kps_2d, confs, idx):
                    has_ankles = True
                    found_points.append(f"Knöchel({idx})[{get_conf(kps_2d, confs, idx):.2f}]")

            # --- Knie / Schenkel (Indizes 9, 12) ---
            has_knees = False
            for idx in [9, 12]:
                if is_valid_point(kps_2d, confs, idx):
                    has_knees = True
                    found_points.append(f"Knie({idx})[{get_conf(kps_2d, confs, idx):.2f}]")
        ...
        # 3. Gewinner finden (Nimmt immer das Maximum, selbst wenn es null Bein-Punkte sind)
        best_idx = int(np.argmax(frame_scores))
        
        if frame_scores[best_idx] < 0:
            return (pose_data_copy, "Fehler: Kein gültiger Frame (mit Torso) im Video gefunden.")
        ...
        for i, meta in enumerate(pose_metas):
            ...
            for idx in range(len(kps)):
                if is_valid_point(kps, confs, idx):
                    valid_y.append(kps[idx][1])
                    valid_x.append(kps[idx][0])
...
# ======================================================================
# 3. V21: GLOBAL PERSPECTIVE SCALER (TOGGLE + PASS-FILTER + 3D FIX)
# ======================================================================
class PoseGlobalPerspectiveScalerV21:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "anchor_window": ("INT", {"default": 2, "min": 0, "max": 15, "step": 1}),
                "min_confidence": ("FLOAT", {"default": 0.3, "min": 0.0, "max": 1.0, "step": 0.05}),
                "frontal_method": (["2D_Ratio", "3D_NLF"], {"default": "2D_Ratio"}),
            },
            "optional": {
                "video_nlf_data": ("NLFPRED",),
            }
    ...
    def process(self, video_pose_data, calibration_data, video_depth_map, include_head, anchor_window, min_confidence, frontal_method="2D_Ratio", video_nlf_data=None):
        import copy
        import numpy as np
        import math
        ...
        pose_data_copy = copy.deepcopy(video_pose_data)
        pose_metas = pose_data_copy.get("pose_metas", [])
        log_messages = ["=== V21 GLOBAL SCALER LOG (TOGGLE & PASS-FILTER) ==="]
        ...
        for i, meta in enumerate(pose_metas):
            ...
            torso_norm = get_torso_norm(kps_2d, confs)
            if kps_2d is None or len(kps_2d) < 14 or torso_norm == 0.0:
            ...
            # Die 2D-Ratio ist am sichersten für den Pass-Filter (0.65 = Frontal Threshold)
            ...
            if is_valid_point(kps_2d, confs, 2) and is_valid_point(kps_2d, confs, 5):
                ...
                frontal_ratio_2d = shoulder_width / torso_norm if torso_norm > 0 else 0.0
                
            is_frontal = frontal_ratio_2d > 0.65  
            ...
            frame_data.append({
                ...
                'has_knees': has_knees,
                'frontal_ratio': frontal_ratio_2d,
        ...
        if has_premium_frontal:
            log_messages.append(">> PASS-FILTER AKTIV: Frontale Frames mit Beinen gefunden! Seitliche Frames werden strikt aussortiert.")
        ...
        for i, data in enumerate(frame_data):
            ...
            # GATING LOGIK
            if has_premium_frontal and not data['is_frontal']:
                frame_scores.append(-1.0)
                score_details.append({"w":0, "f":0, "l":0, "t":-1})
                frame_logs.append(f"Aussortiert! (Seitlich, Ratio: {data['frontal_ratio']:.2f})")
                continue
            ...
            if frontal_method == "3D_NLF" and pose_input_3d is not None and i < len(pose_input_3d):
                ...
                if pose_3d_frame is not None and len(pose_3d_frame) > 0:
                    ...
                    # Dynamische Index-Ermittlung (Verhindert Knie-Bug!)
                    idx_r, idx_l = 2, 5
                    format_name = "OpenPose"
                    if num_joints == 17: # H36M Format
                        idx_r, idx_l = 11, 14
                        format_name = "H36M"
                    elif num_joints in [24, 45, 68]: # SMPL Format
                        idx_r, idx_l = 16, 17
                        format_name = "SMPL"
                    
                    if num_joints > max(idx_r, idx_l):
                        z_right = float(person_3d[idx_r][2])
                        z_left  = float(person_3d[idx_l][2])
                        z_diff = abs(z_left - z_right)
                        frontal_pts = max(0.0, 500.0 - (z_diff * 150.0))
                        used_method_log = f"3D({format_name})"
                    else:
                        frontal_pts = min(500.0, (data['frontal_ratio'] / 0.8) * 500.0)
                        used_method_log = "2D-Fallback"
            else:
                frontal_pts = min(500.0, (data['frontal_ratio'] / 0.8) * 500.0)
                used_method_log = "2D-Ratio"

            # Punkte Länge
            length_pts = (data['length'] / max_body_length) * 100.0
            
            total = bein_pts + frontal_pts + length_pts
            frame_scores.append(total)
            score_details.append({"w": bein_pts, "f": frontal_pts, "l": length_pts, "t": total})
            
            leg_str = "Füße/Knöchel" if waden_pts > 0 else ("Knie" if schenkel_pts > 0 else "Keine Beine")
            frame_logs.append(f"Gültig -> Beine: {leg_str} | Frontal-Wert: {frontal_pts:.1f} [{used_method_log}]")

        best_idx = int(np.argmax(frame_scores))
        
        if frame_scores[best_idx] < 0:
            return (pose_data_copy, "Fehler: Kein gültiger Frame (mit Torso) gefunden.")

        # --- LOGGING ---
        log_messages.append("\n================ PUNKTANALYSE ================")
        log_messages.append(f"Frame 0  -> {frame_logs[0]}")
        log_messages.append(f"Gewinner Frame {best_idx} -> {frame_logs[best_idx]}")
        log_messages.append("==============================================\n")
...
# ======================================================================
# 3. V23: GLOBAL PERSPECTIVE SCALER (TRUE 3D ANGLE CALCULATION)
# ======================================================================
class PoseGlobalPerspectiveScalerV23:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "frontal_3d_angle_tolerance": ("FLOAT", {"default": 20.0, "min": 0.0, "max": 90.0, "step": 1.0, "tooltip": "Maximaler Winkel in Grad (0 = perfekt frontal, 90 = komplett seitlich)"}),
            },
            "optional": {
                "video_nlf_data": ("NLFPRED",),
            }
    ...
    def process(self, video_pose_data, calibration_data, video_depth_map, include_head, anchor_window, min_confidence, frontal_method="3D_NLF", frontal_2d_threshold=0.65, frontal_3d_angle_tolerance=20.0, video_nlf_data=None):
        ...
        pose_data_copy = copy.deepcopy(video_pose_data)
        pose_metas = pose_data_copy.get("pose_metas", [])
        log_messages = ["=== V23 GLOBAL SCALER LOG (3D ANGLE & FOOT BONUS) ==="]
        ...
        for i, meta in enumerate(pose_metas):
            ...
            # --- ZENTRALE FRONTAL-BERECHNUNG ---
            is_frontal = False
            frontal_pts = 0.0
            used_method_log = ""
            
            # 2D Ratio immer als Fallback berechnen
            frontal_ratio_2d = 0.0
            if is_valid_point(kps_2d, confs, 2) and is_valid_point(kps_2d, confs, 5):
                shoulder_width = math.sqrt((kps_2d[2][0] - kps_2d[5][0])**2 + (kps_2d[2][1] - kps_2d[5][1])**2)
                frontal_ratio_2d = shoulder_width / torso_norm if torso_norm > 0 else 0.0
            ...
            if frontal_method == "3D_NLF" and pose_input_3d is not None and i < len(pose_input_3d):
                ...
                if pose_3d_frame is not None and len(pose_3d_frame) > 0:
                    ...
                    if num_joints > max(idx_r, idx_l):
                        # ECHTE WINKELBERECHNUNG (X und Z)
            ...
            frame_data.append({
                ...
                'used_method_log': used_method_log,
                'length': length
            })
        ...
        if has_premium_frontal:
            log_messages.append(f">> PASS-FILTER AKTIV: Frontale Frames (innerhalb Toleranz) mit Beinen gefunden! Seitliche Frames werden aussortiert.")
        ...
        for i, data in enumerate(frame_data):
            if not data['valid']:
                frame_scores.append(-1.0)
                score_details.append({"w":0, "f":0, "fb":0, "l":0, "t":-1})
                frame_logs.append("Ungültiger Torso")
                continue
            ...
            if has_premium_frontal and not data['is_frontal']:
                frame_scores.append(-1.0)
                score_details.append({"w":0, "f":0, "fb":0, "l":0, "t":-1})
                frame_logs.append(f"Aussortiert! (Zu stark gedreht: {data['used_method_log']})")
                continue
            ...
            frontal_pts = data['frontal_pts']

            # Punkte Länge
            length_pts = (data['length'] / max_body_length) * 100.0
            
            total = bein_pts + fuss_bonus_pts + frontal_pts + length_pts
            frame_scores.append(total)
            score_details.append({"w": bein_pts, "fb": fuss_bonus_pts, "f": frontal_pts, "l": length_pts, "t": total})
            
            leg_str = "Füße/Knöchel" if waden_pts > 0 else ("Knie" if schenkel_pts > 0 else "Keine Beine")
            bonus_str = " | +500 Fuß-Bonus!" if fuss_bonus_pts > 0 else ""
            frame_logs.append(f"Gültig -> Beine: {leg_str} | Frontal: {data['used_method_log']}{bonus_str}")
        ...
        log_messages.append(f"-> FRAME 0 INFO: Gesamtpunkte {frame_scores[0]:.1f} (Beine: {score_details[0]['w']:.1f} | Fuß-Bonus: {score_details[0]['fb']:.1f} | Frontal: {score_details[0]['f']:.1f} | Länge: {score_details[0]['l']:.1f})")
        log_messages.append(f"-> GEWINNER INFO (Frame {best_idx}): Gesamtpunkte {frame_scores[best_idx]:.1f} (Beine: {score_details[best_idx]['w']:.1f} | Fuß-Bonus: {score_details[best_idx]['fb']:.1f} | Frontal: {score_details[best_idx]['f']:.1f} | Länge: {score_details[best_idx]['l']:.1f})")
        log_messages.append(f"-> Physische Y-Länge des Gewinners: {body_lengths[best_idx]:.1f}px")
