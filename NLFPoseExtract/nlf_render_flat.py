...
def process_data_to_COCO_format(joints):
    ...
    # --- HIER IST DER FIX ---
    # Wenn das Array 3 Dimensionen hat und die erste eine 1 ist, entfernen wir die 1
    if joints.ndim == 3 and joints.shape[0] == 1:
        joints = joints[0]
    # ------------------------

    # Der originale Check geht jetzt normal weiter und meckert nicht mehr:
    if joints.ndim != 2:
...
# Standard OpenPose Point/Joint Colors
joint_colors_rgb = [
...
def render_nlf_as_images(smpl_poses, dw_poses, height, width, video_length, intrinsic_matrix=None, draw_2d=True, draw_face=True, draw_hands=True, render_backend="taichi"):
    ...
    # Fix: Exact colors, no pastel washout formula, alpha=1.0
    colors = [[c / 255.0 for c in color_rgb] + [1.0] for color_rgb in ordered_colors_255]
    ...
    # Draw 2D joints (points) on top of the 3D rendered body lines
    for i in range(len(frames_np_rgba)):
        frame_img = frames_np_rgba[i]
        if not frame_img.flags.writeable:
            frame_img = frame_img.copy()

        if i < len(smpl_poses) and smpl_poses[i] is not None:
            for joints3d in smpl_poses[i]:
                            ...
                            if np.sum(np.abs(pt3d)) > 0:
                                ...
                                if 0 <= x < width and 0 <= y < height:
                                    color_rgba = (int(joint_colors_rgb[j_idx % 18][0]),
                                                  ...
                                                  255)
                                    cv2.circle(frame_img, (x, y), radius=4, color=color_rgba, thickness=-1, lineType=cv2.LINE_AA)
...
def get_cylinder_specs_list_from_poses(smpl_poses, include_missing=False):
    ...
    # Standard OpenPose Color application for all persons
    standard_colors = [[c / 255.0 for c in color_rgb] + [1.0] for color_rgb in ordered_colors_255]
    ...
    for person_idx in range(max_persons):
        ...
        colors_by_person.append(standard_colors)
